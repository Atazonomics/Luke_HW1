...
a  = {'Spain': [4, 8, 2, 0, 1], 'France': [2, 3, 6],'Italy': [6, 8, 1, 7]}
param = 'Spain'

def total_registered_cases(df,country):
    new_dic = {}
    for k,v in  a.items():
        if k == param:
            tot = sum(v)
...
total_registered_cases(a,param)
...
def total_registered_cases_per_country(df):
    new_dict = {}
    for k,v in a.items():
        new_dict[k] = sum(v)
    return new_dict

total_registered_cases_per_country(a)
...
a  = {'Spain': [4, 8, 2, 0, 1], 'France': [2, 3, 6],'Italy': [6, 8, 1, 7],'USA': [20, 8, 1, 7]}

def country_with_most_cases(df):
    b = 0
    new_dict = {}
    for  k,v in df.items():
        new_dict[k] = sum(v)
